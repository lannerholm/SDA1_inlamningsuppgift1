...
Figurerna och fördelningsmåtten ger oss en bild av ett mestadels fridfullt Boston, där kriminaliteten är koncentrerad till ett relativt fåtal områden, s.k. hot-spots. Varför brottsligheten är koncentrerad i just de områdena vet vi inte än, men vi kan spekulera i att det kanske beror på att det i de områdena förekommer nattliv (dvs. är nära stadskärnan), eller att sammhällets utslagna bor där (dvs. låg median_home_value).

```{r}
favstats(Boston_census_data$crime_rate)
hist(Boston_census_data$crime_rate, breaks = 50)
boxplot(Boston_census_data$crime_rate)
...
(förutsatt att vi inte nämner statistisk signifikans) så ser vi ett samband mellan town/crime rate i jämförandet mellan Cambridge och övriga städer. Om vi enbart jämför de 4 andra städerna mot varandra ser vi inte ett lika uppenbart samband.
...
Eftersom korrelation bara gills för numeriska variabler tog vi först bort de som R ansåg vara kategoriska med hjälp av funktionen 'select_if(is.numeric)'.
...
boxplot(Boston_census_data$tax_rate)
...
Vi undrar vad andelen av varje skattekategori är givet borders_charles. Tabellen och figuren visar endast ett svagt samband mellan vilken skattekategori ett distrikt har, och dess angränsning till Charles River. Bland de distrikt som ligger i anslutning, har mindre en andel låg och hög skattekategori, än de distrikt som inte ligger i anslutning.
...
Andra frågan handlar om procentandelan betingat på skattekategori. Därför kan vi avläsa svaret i tabellen med kolumnprocent. Vi ser att 94.7% av censusdistrikten med hög skatt inte ligger i angränsning till Charles River.
...
#Vi ser ett mycket svagt negativt samband,med styrkan -0.002.
plot(Boston_census_data$dist_fenway_park, Boston_census_data$median_home_value)
#plotten visar inte heller något samband, så vi kan med ganska god säkerhet säga att det inte finns något meningsfullt samband mellan de två variablerna.
```
:::
...
\# residual vs. fitted visar ett svagt icke-linjärt samband.
...
Vi plottade de enskilda variablerna mot median_home_value för att se om där var några icke-linjära samband. Vi mörkte att before_1940_p var något böjd och gjorde en transformation med y =\> y\^2.
...
Pubktdiagrammet för när y är kvadrerad är mer linjär. Vi kör regressionen igen och ser om förklaringsgraden ökar.
...
Vi såg en liten förbättring av Adjusted R-squared, från 0.3988 till 0.4013. Vi korsvaliderar nu våran modell.
...
Vi såg också en liten förbättring i RMSE, från 5.6556 till 5.567.
:::
...
Vi ser på våran qqplot att den är hyfsat linjär, och ser bra ut mellan -1 och 1. Därefter blir den snabbt mycket sämre.
...
predict(multipleModel2, Boston_districts_to_predict)
predict(multipleModel, Boston_districts_to_predict)
```
:::
