...
Vi ser att brottsligheten i en betydande majoritet av distrikt är låg, men ett fåtal distrikt har extremt hög kriminalitet. Troligtvis rör sig outliers inte om en felskrivning: värdena är korrekta.

Då de outliers som finns har extremt höga värden kan det vara bättre att rapportera på IRQ/medianen istället för Standardavikelssen /medelvärdet, då de förstnämnda är mindre känsliga för extremvärden. Dock skulle det rent praktiskt vara något "oärligt" att ignorera extremvärdena då det i detta dataset är vad som är det intressanta att analysera.
...
Skriv svaret här. De tre variabler som hade högst korrelationskoffecient gentemot crime_rate är, i ordning störst \> minst: radial_access, tax_rate, och lower_stat_pct.

```{r}
strictly_numerical <- Boston_census_data %>%
  select_if(is.numeric) %>%
  select(-any_of(c("longitude", "latitude", "borders_charles", "n_rooms_avg"))) #finns här kategoriska variabler kvar?
